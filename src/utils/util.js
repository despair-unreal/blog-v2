export default {
  ...
  changeColorFormat: function (finalFormat, headStr, headStrFormat) {
    let result;
    //获取颜色值格式
    if (!headStrFormat) headStrFormat = this.getStrColorFormat(headStr);
    switch (finalFormat) {
      case 'html':
      ...
      case 'rgba': {
        ...
        if (finalFormat === 'rgba') result = `rgba(${rgb[0]},${rgb[1]},${rgb[2]},${a})`;
        else result = `rgb(${rgb[0]},${rgb[1]},${rgb[2]})`;
        break;
      }
    }
    return result;
